...
| `memory/mnesis.ts` | hermetic | `mnesis.test.ts`, `conformance.test.ts`, `server/memory-provider.test.ts` | `Bun.serve` MCP stub emulating the memory-server Streamable HTTP wire contract: first-class field mapping (session_id / event_type / learning / neurons / operation_id / context), deterministic session-id & record-id derivation, wrong-tenant 401 fail-closed, permission-filtered read-only surface (`-32602`), forget/digest-pruning loud rejection (no delete tool, issue #348 verdict), consolidation on-save; full conformance suite against the stub. Provider selection covers `memory_backend.kind=mnesis` + vault token/tenant and unconfigured fail-closed. | No action — at tier. The real memory-server leg is documented separately (BOTTEGA_RUN_INTEGRATION, not yet present). |
| `memory/conformance.test.ts` | test infra | run by `sqlite.test.ts` + `mem0.test.ts` + `mnesis.test.ts` | Shared suite: org round-trip, org sharing, principal isolation, principal requirement, limit cap, metadata exact filter, no delete/update surface. | No action — the pattern (external backends run it against wire stubs) is correct. |
...
| local bootstrap (`scripts/dev.sh`, `scripts/dev-bootstrap.ts`) | caller-level hermetic + structural | `scripts/dev-bootstrap.test.ts`, `egress/dev-compose.test.ts` | Shared setup/dev orchestration is exercised through injected command, filesystem, clock, and probe ports: preview zero-mutation, explicit apply, partial/rerun idempotency, no-clobber owner/mode checks, Docker/native failures, proxy timeout/stale recovery, broker fallback/token readiness, declared file modes, redacted output, and exact server argv/env. The shell test asserts only thin canonical-path delegation. | No action; live Docker/image behavior remains in the existing skip-gated proxy/e2e lanes. |
...
8. **Local bootstrap caller coverage closed (#311).** Preview/apply/dev and failure recovery now use the same injected orchestration.
...
| 8 | Local bootstrap untested | Replaced the mutating shell orchestration with a thin launcher plus `scripts/dev-bootstrap.ts`. Caller-level hermetic tests cover preview/apply/dev decisions, failure deadlines, file safety, fallback, redaction, and final process contract. | `scripts/dev-bootstrap.test.ts` |
