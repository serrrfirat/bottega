 ...
 * stay hardcoded here; extension domains append to them, deduped. Default-deny
 * enforcement is unchanged: direct content-bearing extension domains pass the
 * judge transform, while reviewed MCP transport domains remain allowlist +
 * auth + audit gated.
 * The DEV config is the ONLY permissive surface: allow-all
 ...
 * transport hosts (model gateways, Slack, api.github.com, and pinned MCP
 * domains) are allowlist+secrets+audit gated and deliberately NOT judged —
...
#   2. judge      — direct content hosts (search and KB fetch) are
#                   policy-judged by an LLM ("deny unless clearly required
#                   by the task and safe"). Fallback deny keeps egress
#                   closed when the judge LLM is down or the circuit breaker
#                   is open. Infrastructure and reviewed MCP transport hosts
#                   are code-pathed, allowlisted, credential-targeted, and
#                   audited — an LLM URL-only verdict on their CONNECT /
#                   tools/list handshakes is noise (verified live in #371),
#                   so they are judged no more.
...
transforms:
  ...
  #    *.slack.com, file downloads on files.slack.com), plus reviewed extension
  #    domains from config/extensions snapshots (issue #50). Every destination
  #    must be allowlisted; only the direct content hosts below are also judged.
...
${domainLines}
  ...
  # 2. Judge: LLM policy gate on direct content traffic only (search, KB
  #    fetch). Infrastructure and reviewed MCP transport hosts are exempt —
  #    see the design note above (#371).
  - name: judge
    config:
      name: "egress-policy"
