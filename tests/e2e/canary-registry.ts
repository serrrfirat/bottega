...
export const JOURNEYS: readonly CanaryJourney[] = [
  ...
  },
  {
    ...
    layer: "hermetic",
    actors: ["requester", "member", "second-member"],
    covers: ["read", "glob", "grep", "ast_grep", "lsp", "task", "use_model"],
  ...
  // an explicit non-empty exclusionReason documents a class covered elsewhere.
  {
    ...
    layer: "hermetic",
    actors: [],
    covers: ["read", "glob", "grep", "ast_grep", "web_search", "inspect_image", "lsp", "task", "use_model", "model_settings"],
  ...
  },
  {
    ...
    layer: "hermetic",
    actors: [],
    covers: ["create_scheduler_job", "list_scheduler_jobs", "delete_scheduler_job", "kb_ingest", "list_todos", "complete_work_item", "search_web"],
