...
**Any change to the live-API leg's egress requires the runner to boot
iron-proxy (issue #304).** The `live-api` job runs on `ubuntu-latest` where
no proxy is running, so it boots an **ephemeral iron-proxy** via
`bun run scripts/canary-proxy-boot.ts` (`docker-compose.dev.yml`: the same
permissive dev config `config/egress.dev.yml` local dev rides — allow-all +
no judge, secrets kept) with the MITM CA generated by the pinned
**public** Docker Hub image
[`ironsh/iron-proxy:0.49.0`](https://hub.docker.com/r/ironsh/iron-proxy)
(`docker run --rm -v $PWD/certs:/certs ironsh/iron-proxy:0.49.0
generate-ca -outdir /certs` — the same command `dev.sh` uses). The image is
**public**, so no registry login is needed on the runner; `docker pull`
just works (the CI `integration` job already pulls it the same way). The
boot step seeds the proxy's `<provider>.secret` files from the **model-gateway
repository secrets** (`NEAR_API_KEY` etc., issue #208) so the proxy injects
the real bearer at egress, then an `if: always()` step tears the proxy
down.
